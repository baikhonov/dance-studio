...
export function AdminPage() {
  ...
  if (isLoading) {
    return <p className="text-slate-600">Загрузка админки...</p>
  }
  ...
  return (
    <div className="space-y-8 py-2">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3 xl:gap-8">
        <section className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm md:p-5">
          <div className="mb-4 flex flex-col gap-3">
            <h2 className="text-xl font-semibold text-gray-900">Настройки школы</h2>
            <p className="text-sm text-gray-600">Название используется в шапке, футере и title страницы</p>
          </div>

          <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
            <label className="block">
              <span className="mb-1 block text-sm text-gray-700">Название школы</span>
              <input
                type="text"
                value={schoolNameDraft}
                onChange={(event) => setSchoolNameDraft(event.target.value)}
                maxLength={80}
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 md:text-base"
              />
            </label>
            <button
              type="button"
              ...
              className="rounded-lg bg-amber-500 px-4 py-2 text-sm text-white hover:bg-amber-600 disabled:opacity-60 md:text-base"
            >
          ...
          <div className="mt-4 flex flex-col gap-3">
            <p className="text-sm text-gray-700">Логотип</p>
            <div className="flex flex-wrap items-center gap-3">
              {logoPreviewUrl ? (
                <img
                  src={logoPreviewUrl}
                  alt="Логотип школы"
                  className="h-14 max-w-[180px] rounded border border-gray-200 bg-white object-contain p-1"
                />
              ) : (
                <span className="text-sm text-gray-500">Логотип не загружен</span>
              )}
          ...
          {settingsMessage && <p className="mt-3 text-sm text-emerald-600">{settingsMessage}</p>}
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm md:p-5">
          <div className="mb-4 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-gray-900">Управление преподавателями</h2>
              <span className="rounded-full bg-blue-100 px-2.5 py-1 text-xs font-medium whitespace-nowrap text-blue-700 md:text-sm">
                Всего: {teachers.length}
              </span>
            </div>
            <p className="text-sm text-gray-600">Состав и карточки преподавателей</p>
          </div>

          <button
            type="button"
            onClick={() => openTeacherModal()}
            className="mb-4 rounded-lg bg-amber-500 px-3.5 py-2 text-sm text-white hover:bg-amber-600 md:text-base"
          >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {teachers.map((teacher) => (
              <button
                key={teacher.id}
                type="button"
                className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 bg-white p-3 text-left transition hover:border-blue-200"
              ...
              >
                ...
                />
                <p className="text-base leading-tight font-semibold text-gray-900">{teacher.name}</p>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm md:p-5">
          <div className="mb-4 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-gray-900">Управление направлениями</h2>
              <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium whitespace-nowrap text-emerald-700 md:text-sm">
                Всего: {directions.length}
              </span>
            </div>
            <p className="text-sm text-gray-600">Справочник направлений для расписания</p>
          </div>
          ...
          <ul className="space-y-2">
            {directions.map((direction) => (
              <li key={direction.id}>
                <button
                  type="button"
                  className="flex min-h-11 w-full cursor-pointer items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-left transition hover:border-emerald-200 hover:bg-emerald-50/40"
                  onClick={() => openDirectionModal(direction)}
                >
                  <span className="leading-tight font-medium text-gray-800">{direction.name}</span>
        ...
        <section className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm md:p-5">
          <div className="mb-4 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-gray-900">Управление уровнями</h2>
              <span className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-medium whitespace-nowrap text-violet-700 md:text-sm">
                Всего: {managedLevels.length}
              </span>
            </div>
            <p className="text-sm text-gray-600">Единый справочник уровней занятий</p>
          </div>
          ...
          <ul className="space-y-2">
            {managedLevels.map((level) => (
              <li key={level.id}>
                <button
                  type="button"
                  className="flex min-h-11 w-full cursor-pointer items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-left transition hover:border-violet-200 hover:bg-violet-50/40"
                  onClick={() => openLevelModal(level)}
                >
                  <span className="inline-flex items-center gap-2 leading-tight font-medium text-gray-800">
